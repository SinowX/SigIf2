...
static bool CheckIfQuery(nlohmann::basic_json<>& rqs)
{
...
static bool CheckIfSetting(nlohmann::basic_json<>& rqs)
{
	if(rqs["meta"]["action"].get<std::string>()
...
// check json if valid
static bool CheckIfValid(nlohmann::basic_json<>& rqs){
...
// return instruction
static std::string GetInstruction(nlohmann::basic_json<>& rqs){
		InsPack packer;
		if(rqs["meta"]["type"].get<std::string>().compare(IFTYPE::ActionTable))
		{
			if(CheckIfQuery(rqs))
...
#include <bitset>
// return json str
static std::string GetResponse(std::shared_ptr<InsParser> parser, std::string ipv4){
	/* InsParser parser; */
