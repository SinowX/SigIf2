...
int accept_fd_hdlr(const int fd)
{
	...
	ConnInfoPtr newconn = std::make_shared<ConnInfo>(
			ConnType::TCP, true, newfd,
			inet_ntoa(addr.sin_addr),	ntohs(addr.sin_port)
			);
	
	ConnMap::getInstance().Insert(inet_ntoa(addr.sin_addr),newconn);
...
{
	ConnInfoPtr conn = ConnMap::getInstance().Find(fd);
	
	switch (conn->get_ConnType())
	{
		case ConnType::TCP:
			{
...
{
	auto pack = machine_conn->get_Udp_Conn()->try_Get_Pack();
	InsParser parser;
